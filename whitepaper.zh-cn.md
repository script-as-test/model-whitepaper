...
那么，我们如何让自动化测试真正融入协作流程？如何让测试资产从“报告”变成“资源”？这就是本白皮书要解答的问题 —— 我们提出了一种模型：**Script-as-Test（SAT 模型）**。
...
从“导入报告”改为从“同步脚本”，这就是 Script-as-Test 的起点。
...
**SAT 提供的，是一个“用脚本构建协作资产”的思路**，让自动化不再只是执行，而成为作的一部分。
...
QA 可以对接自动化脚本进行评审与补充， 开发团队也能更清晰地了解，哪些功能点已有对应测试脚本，哪些尚未覆盖，PM 可以看到完整的覆盖范围与来源路径。
...
Script-as-Test 模型由 Will Shi 原创提出，旨在解决当前测试管理中的协作盲区，它让测试脚本可被看见、可被审计、可被管理，这是对当下测试资产沉没问题的一种回应。 

它试图在技术日益自动化的同时，让协作关系也更清晰、信息更可溯源、用例更可维护。

它并不复杂，也不试图替代任何一环，而是在既有工具和实践中，加上一层结构，加上一个看见的视角。

希望本白皮书能帮助你重新理解“测试资产”的含义，思考自动化之外的“管理价值”，并在适合的团队中尝试落地。
...
Script-as-Test 模型由 SAT 框架的创始人 Will Shi (石巍 / SHI WEI) 提出。

本白皮书版权归作者 Will Shi (石巍 / SHI WEI) 所有，欢迎引用、转发，但请注明出处。
...
不需要，插件基于 Forge，无需本地部署。但当前正在申请 Atlassian Marketplace 公开上架，上架后可以通过在 Marketplace 搜索 AutoTestCase Extractor，直接安装即可；若因国际形势等问题无法上架，可联系 will.shi@tman.ltd 申请。插件说明：https://docs.tman.ltd/atce/overview
